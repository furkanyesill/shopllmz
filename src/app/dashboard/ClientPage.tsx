...
export default function ClientPage({ initialShop, initialIsPro }: { initialShop: string | null, initialIsPro: boolean }) {
  ...
  useEffect(() => {
    ...
    // Fallback if initialShop was empty (unlikely with Server Components)
    if (!initialShop) {
      const shopifyShop = (window as any).shopify?.config?.shop;
      if (shopifyShop) setActiveShop(shopifyShop);
    }
  }, [initialShop]);
